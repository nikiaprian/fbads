...
function App() {
  const [accessToken, setAccessToken] = useState(process.env.REACT_APP_FB_ACCESS_TOKEN);
  const [adAccountId, setAdAccountId] = useState(process.env.REACT_APP_AD_ACCOUNT_ID);
  ...
  const formatBudget = (budget) => (budget ? `${parseInt(budget) / 100} IDR` : "-");
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white p-6 rounded-xl shadow-md mb-6">
          ...
          <button
            ...
            disabled={loading}
            className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full transition duration-200 shadow mr-4"
          >
          ...
          <button
            ...
            disabled={loading}
            className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-full transition duration-200 shadow"
          >
        ...
        {insightsCountry.length > 0 && (
          <Section title="Insights per Negara (April 2024)">
            <table className="min-w-full text-sm text-left text-gray-700">
              <thead className="bg-green-600 text-white uppercase text-xs">
                <tr>
                  ...
                  <th className="px-5 py-3">Spend (USD)</th>
                  <th className="px-5 py-3">CTR</th>
                  <th className="px-5 py-3">CPM</th>
                </tr>
              </thead>
              <tbody>
                {insightsCountry.map((insight, idx) => (
                  <tr key={idx} className={idx % 2 === 0 ? "bg-white" : "bg-green-50"}>
                    <td className="px-5 py-2">{insight.country}</td>
                    <td className="px-5 py-2">{insight.impressions}</td>
                    <td className="px-5 py-2">{insight.clicks}</td>
                    <td className="px-5 py-2">{insight.spend}</td>
                    <td className="px-5 py-2">{insight.ctr}</td>
                    <td className="px-5 py-2">{insight.cpm}</td>
                  </tr>
                ))}
              </tbody>
            </table>
